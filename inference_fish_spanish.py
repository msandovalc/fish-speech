import os
import sys
import torch
import shutil
import random
import numpy as np
import soundfile as sf
from pathlib import Path
from loguru import logger
from datetime import datetime

# --- CONFIGURACIÓN DE LOGS TRACE ---
logger.remove()
logger.add(
    sys.stdout,
    colorize=True,
    level="TRACE",
    format="<green>{time:HH:mm:ss}</green> | <level>{level: <8}</level> | <cyan>{message}</cyan>"
)

PROJECT_ROOT = Path("/kaggle/working/fish-speech")
os.environ["EINX_FILTER_TRACEBACK"] = "false"

from fish_speech.inference_engine import TTSInferenceEngine
from fish_speech.models.dac.inference import load_model as load_decoder_model
from fish_speech.models.text2semantic.inference import launch_thread_safe_queue
from fish_speech.utils.schema import ServeTTSRequest, ServeReferenceAudio


class FishSimpleLab:
    def __init__(self):
        self.device = "cuda"
        self.checkpoint_dir = PROJECT_ROOT / "checkpoints" / "openaudio-s1-mini"
        self.precision = torch.half

        logger.info("🚀 CARGANDO MODELOS (MODO SIMPLE)...")
        self.engine = self._load_models()

    def _load_models(self):
        llama_queue = launch_thread_safe_queue(checkpoint_path=self.checkpoint_dir, device=self.device,
                                               precision=self.precision, compile=True)
        decoder_model = load_decoder_model(config_name="modded_dac_vq",
                                           checkpoint_path=self.checkpoint_dir / "codec.pth", device=self.device)
        return TTSInferenceEngine(llama_queue=llama_queue, decoder_model=decoder_model, precision=self.precision,
                                  compile=True)

    def run_50_variants(self, text, ref_path):
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        folder = PROJECT_ROOT / f"mega_test_{timestamp}"
        folder.mkdir(parents=True, exist_ok=True)

        with open(ref_path, "rb") as f:
            ref_bytes = f.read()

        logger.info(f"🧪 Generando 50 variantes en: {folder}")

        for i in range(1, 51):
            # Parámetros aleatorios SEGUROS (Temp <= 1.0)
            t = round(random.uniform(0.3, 1.0), 2)
            p = round(random.uniform(0.6, 0.9), 2)
            penalty = round(random.uniform(1.1, 1.4), 2)
            chunk = random.choice([250, 350, 500])

            name = f"V{i:02d}_T{t}_P{p}_Pen{penalty}_C{chunk}"
            logger.trace(f"🌀 [{i}/50] -> {name}")

            req = ServeTTSRequest(
                text=text,
                references=[ServeReferenceAudio(audio=ref_bytes, text="")],
                max_new_tokens=1024,
                chunk_length=chunk,
                top_p=p,
                temperature=t,
                repetition_penalty=penalty,
                format="wav"
            )

            try:
                results = self.engine.inference(req)
                audio_chunks = []
                for res in results:
                    item = res.audio if hasattr(res, 'audio') else res
                    if isinstance(item, tuple):
                        for sub in item:
                            if isinstance(sub, np.ndarray): audio_chunks.append(sub)
                    elif isinstance(item, np.ndarray):
                        audio_chunks.append(item)

                if audio_chunks:
                    final_audio = np.concatenate(audio_chunks)
                    sf.write(str(folder / f"{name}.wav"), final_audio, 44100)
                    logger.debug(f"✅ Guardado {name}")
            except Exception as e:
                logger.error(f"❌ Error en variante {i}: {e}")

        # Empaquetado final
        zip_name = f"resultados_50_{timestamp}"
        shutil.make_archive(str(PROJECT_ROOT / zip_name), 'zip', folder)
        logger.success(f"🏁 ZIP LISTO: {zip_name}.zip")


if __name__ == "__main__":
    lab = FishSimpleLab()
    TEXTO = "La mente es la causa de todo; produce la realidad del individuo, ¡con total claridad!"
    REFERENCIA = "/kaggle/working/fish-speech/voice_to_clone.wav"
    lab.run_50_variants(TEXTO, REFERENCIA)